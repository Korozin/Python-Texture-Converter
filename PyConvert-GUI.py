...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Name and create the Main Window
        ...
        # Initialize Timer for use with real-time functions
        self.timer = QTimer()
        self.timer.start(1000) # Set timer to 1 second
        self.timer.timeout.connect(self.update_fallback_label) # Update "fallback" label with timer
    ...
    def Create_Conversion_Tab(self):
        ...
        # Create input .zip line edit
        self.input_zip_edit = QLineEdit()
        self.input_zip_edit.setPlaceholderText("Zipped Texture Pack Location")
        self.input_zip_edit.setToolTip("Set the location of the zipped Texture Pack")
        
        # Create input .zip browse button
        self.input_zip_button = QPushButton("...")
        self.input_zip_button.setToolTip("Use the GUI to select zipped Texture Pack")
        self.input_zip_button.clicked.connect(self.select_texture_zip)
        self.input_zip_button.setFixedWidth(40)
        
        # Create output folder line edit
        self.output_folder_edit = QLineEdit()
        self.output_folder_edit.setPlaceholderText("Output Location")
        self.output_folder_edit.setToolTip("Set the location of the output folder")
        
        # Create output folder browse button
        self.output_folder_button = QPushButton("...")
        self.output_folder_button.setToolTip("Use the GUI to select output folder")
        self.output_folder_button.clicked.connect(self.select_output_folder)
        self.output_folder_button.setFixedWidth(40)
        
        # Create start button on Conversion tab
        self.start_button = QPushButton("Start")
        self.start_button.setToolTip("Start Conversion")
        self.start_button.clicked.connect(self.start_conversion)
        
        # Create Conversion Tab layouts
        conversion_tab_layout = QVBoxLayout()
        input_layout = QHBoxLayout()
        output_layout = QHBoxLayout()
        
        # Add Widgets to respective layouts
        input_layout.addWidget(self.input_zip_edit)
        input_layout.addWidget(self.input_zip_button)
        output_layout.addWidget(self.output_folder_edit)
        output_layout.addWidget(self.output_folder_button)
        
        # Add other layouts to Conversion Tab's main layout
        conversion_tab_layout.addLayout(input_layout)
        conversion_tab_layout.addLayout(output_layout)
        conversion_tab_layout.addWidget(self.start_button)
        
        # Set Conversion Tab's main layout
        self.conversion_tab.setLayout(conversion_tab_layout)
    ...
    def Create_Options_Tab(self):
        ...
        self.fallback_checkbox = QCheckBox("Use fallback", self.options_tab)
        self.fallback_checkbox.setToolTip("Select whether or not fallback images are used")
        self.fallback_checkbox.move(50, 50)
        self.fallback_checkbox.setChecked(True)

        # Create items checkbox
        self.items_checkbox = QCheckBox("Convert items", self.options_tab)
        self.items_checkbox.setToolTip("Select whether or not Items are converted")
        self.items_checkbox.move(190, 50)
        self.items_checkbox.setChecked(True)

        # Create blocks checkbox
        self.blocks_checkbox = QCheckBox("Convert blocks", self.options_tab)
        self.blocks_checkbox.setToolTip("Select whether or not Blocks are converted")
        self.blocks_checkbox.move(330, 50)
        self.blocks_checkbox.setChecked(True)

        # Create armor checkbox
        self.armor_checkbox = QCheckBox("Convert armor", self.options_tab)
        self.armor_checkbox.setToolTip("Select whether or not Armor is converted")
        self.armor_checkbox.move(50, 100)
        self.armor_checkbox.setChecked(True)

        # Create environment checkbox
        self.environment_checkbox = QCheckBox("Convert envir..", self.options_tab)
        self.environment_checkbox.setToolTip("Select whether or not Environment files are converted")
        self.environment_checkbox.move(190, 100)
        self.environment_checkbox.setChecked(True)

        # Create particles checkbox
        self.particles_checkbox = QCheckBox("Convert particles", self.options_tab)
        self.particles_checkbox.setToolTip("Select whether or not Particles are converted")
        self.particles_checkbox.move(330, 100)
        self.particles_checkbox.setChecked(True)

        # Create fire checkbox
        self.fire_checkbox = QCheckBox("Convert fire", self.options_tab)
        self.fire_checkbox.setToolTip("Select whether or not Fire is converted")
        self.fire_checkbox.move(50, 150)
        self.fire_checkbox.setChecked(True)

        # Create misc checkbox
        self.misc_checkbox = QCheckBox("Convert misc", self.options_tab)
        self.misc_checkbox.setToolTip("Select whether or not misc items are converted")
        self.misc_checkbox.move(190, 150)
        self.misc_checkbox.setChecked(True)
        
        # Create items res Label
        self.items_res_label = QLabel("Items Res:", self.options_tab)
        self.items_res_label.move(60, 191)

        # Create items res line edit
        self.items_res_edit = QLineEdit(self.options_tab)
        self.items_res_edit.setPlaceholderText("Item res (ex: 16 = 16x16)")
        self.items_res_edit.setToolTip("Set the resolution of the Items (16 by default)")
        self.items_res_edit.setText("16")
        self.items_res_edit.resize(220, 20)
        self.items_res_edit.move(150, 190)
        
        # Create blocks res Label
        self.blocks_res_label = QLabel("Blocks Res:", self.options_tab)
        self.blocks_res_label.move(60, 221)
        
        # Create blocks res line edit
        self.blocks_res_edit = QLineEdit(self.options_tab)
        self.blocks_res_edit.setPlaceholderText("Blocks res (ex: 16 = 16x16)")
        self.blocks_res_edit.setToolTip("Set the resolution of the Blocks (16 by default)")
        self.blocks_res_edit.setText("16")
        self.blocks_res_edit.resize(220, 20)
        self.blocks_res_edit.move(150, 220)

        # Create download fallback button
        self.download_fallback_button = QPushButton("Download Fallback", self.options_tab)
        self.download_fallback_button.setToolTip("Downloads and unzips fallback.zip")
        self.download_fallback_button.clicked.connect(self.download_fallback)
        self.download_fallback_button.move(30, 260)
        self.download_fallback_button.resize(455, 30)

        # Create fallback.zip detected state label
        self.fallback_exists_label = QLabel("fallback : <span style='color:red'>missing</span>", self.options_tab)
        self.fallback_exists_label.resize(200, 30)
        self.fallback_exists_label.move(190, 330)
    ...
    def Create_Log_Tab(self):
        ...
        # Make a QFont instance
        font = QFont()
        font.setPointSize(8)
        
        # Create log text edit
        self.log_text_edit = QTextEdit()
        self.log_text_edit.setStyleSheet("background-color: #0f0f0f;")
        self.log_text_edit.verticalScrollBar().setStyleSheet("background-color: lightGray;")
        ...
        self.log_start_button = QPushButton("Start")
        self.log_start_button.setToolTip("Starts Conversion")
        self.log_start_button.clicked.connect(self.start_conversion)
        
        # Create Log tab's main layout
        log_tab_layout = QVBoxLayout()
        
        # Add widgets to Log tab's main layout
        log_tab_layout.addWidget(self.log_text_edit)
        log_tab_layout.addWidget(self.clear_log_button)
        log_tab_layout.addWidget(self.log_start_button)
        
        # Set Log tab's main layout
        self.log_tab.setLayout(log_tab_layout)
        # --- Log Tab Items End --- #
        

    # Function to get ZIP Path
    def select_texture_zip(self):

        # Create a dialog to allow the user to select their ZIP file
        ...
        if file_dialog.exec_():
            self.input_zip_edit.setText(file_dialog.selectedFiles()[0])
        else:
            self.input_zip_edit.setText("")
    ...
    # Function to call conversion logic based on checkbox state
    def start_conversion(self):
        self.clear_log()
    
        if self.input_zip_edit.text() == "":
            # Create QMessageBox dialog based on empty input field
            input_error_box = QMessageBox()
            input_error_box.setIcon(QMessageBox.Warning)
            input_error_box.setWindowTitle("Error!")
            input_error_box.setText(f"<br/>Emtpy input field!<br/><br/>Make sure to populate the Zipped Texture input!")
            input_error_box.setStandardButtons(QMessageBox.Ok)
        
            # Create custom Check mark icon with green coloring
            check_icon = QPixmap(16, 16)
            check_icon.fill(Qt.transparent)
            painter = QPainter(check_icon)
            pen = QPen(QColor('#00f500'))
            painter.setPen(pen)
            ...
            completion_box.setIcon(QMessageBox.Information)
            completion_box.setWindowTitle("Completed!")
            completion_box.setText(f"<br/>Process completed!")
            completion_box.setStandardButtons(QMessageBox.Ok)
            ...
            completion_box.exec_()
            
            
    # Function to convert the items from the "items" folder
    def convert_items(self):

        self.calculate_items_size(int(self.items_res_edit.text()))

        if self.output_folder_edit.text() == "":
            selected_output = "output"
        else:
            selected_output = self.output_folder_edit.text()

        # create output folder if it doesn't exist
        if not os.path.exists(selected_output):
            os.mkdir(selected_output)

        # create a new transparent image with user size calculated from res
        items = Image.new('RGBA', (self.items_width, self.items_height), (0, 0, 0, 0))
        ...
        for i, filename in enumerate(items_filenames):
            x = (i % 16) * int(self.items_res_edit.text())
            y = (i // 16) * int(self.items_res_edit.text())
            try:
                img = Image.open(f'tmp/assets/minecraft/textures/items/{filename}.png')
                img = img.resize((int(self.items_res_edit.text()), int(self.items_res_edit.text())))
                items.paste(img, (x, y))
    ...
    def convert_blocks(self):
        ...
        self.calculate_blocks_size(int(self.blocks_res_edit.text()))
        ...
        terrain = Image.new('RGBA', (self.blocks_width, self.blocks_height), (0, 0, 0, 0))
        ...
        for i, filename in enumerate(terrain_filenames):
            x = (i % 16) * int(self.blocks_res_edit.text())
            y = (i // 16) * int(self.blocks_res_edit.text())
            try:
                img = Image.open(f'tmp/assets/minecraft/textures/blocks/{filename}.png')
                img = img.resize((int(self.blocks_res_edit.text()), int(self.blocks_res_edit.text())))
        ...
        # save the new image to the output folder
        terrain.save(f'{selected_output}/terrain.png')
    
        # resize image and save
        width = int(self.blocks_width / 2)
        height = int(self.blocks_height / 2)
        mipMapLevel2 = terrain.resize((width, height))
        mipMapLevel2.save(f'{selected_output}/terrainMipMapLevel2.png')
    
        # resize image and save
        width = int(self.blocks_width / 4)
        height = int(self.blocks_height / 4)
        mipMapLevel3 = terrain.resize((width, height))
    ...
    def convert_armor(self):
        ...
        # create output folder if it doesn't exist
        try:
            if not os.path.exists(f'{selected_output}'):
                os.mkdir(f'{selected_output}')
                os.mkdir(f'{selected_output}/armor')
            else:
                os.mkdir(f'{selected_output}/armor')
        ...
        output_dir = f'{selected_output}/armor'
    ...
    def convert_environment(self):
        ...
        try:
            if not os.path.exists(f'{selected_output}'):
                os.mkdir(f'{selected_output}')
                os.mkdir(f'{selected_output}/environment')
                os.mkdir(f'{selected_output}/terrain')
            else:
                os.mkdir(f'{selected_output}/environment')
                os.mkdir(f'{selected_output}/terrain')
        except FileExistsError:
        ...
        for filename in filenames:
            ...
            if os.path.exists(source_path):
                ...
                if filename in ['clouds', 'rain', 'snow']:
                    dest_path = os.path.join(f'{selected_output}', 'environment', filename + '.png')
                else:
    ...
    def convert_particles(self):
        ...
        output_folder = f'{selected_output}'
    ...
    def convert_fire(self):
        ...
        try:
            if not os.path.exists(f'{selected_output}'):
                os.mkdir(f'{selected_output}')
                os.mkdir(f'{selected_output}/fire')
            else:
                os.mkdir(f'{selected_output}/fire')
        except FileExistsError:
            pass

        # Set directories used in conversion
        source_dir = 'tmp/assets/minecraft/textures/blocks'
        fallback_dir = 'fallback/fire'
        output_dir = f'{selected_output}/fire'
    ...
    def convert_misc(self):
        ...
        try:
            if not os.path.exists(f'{selected_output}'):
                os.mkdir(f'{selected_output}')
                os.mkdir(f'{selected_output}/misc')
            else:
                os.mkdir(f'{selected_output}/misc')
        except FileExistsError:
            pass
        ...
        output_folder = f'{selected_output}/misc'
    ...
    # Function to update the fallback label based on directory validity
    def update_fallback_label(self):
        if not os.path.exists("fallback"):
            self.fallback_exists_label.setText("fallback : <span style='color:red'>missing</span>")
        else:
            self.fallback_exists_label.setText("fallback : <span style='color:#00f500'>detected</span>")
            

    # Function to show dialog on-launch if fallback directory does not exist
    def fallback_on_launch(self):
        if os.path.isdir("fallback"):
            pass
        else:
            # Create and center error box is fallback directory does not exist
            fallback_box = QMessageBox()
            fallback_box.setIcon(QMessageBox.Warning)
            fallback_box.setWindowTitle("Error!")
            fallback_box.setText("<br/>Fallback directory not detected!<br/><br/>Would you like to download it now?")
            fallback_box.show()
            fallback_box.move(MainWindow().geometry().center() - fallback_box.rect().center())
            ...
            # Create custom X mark icon with red coloring
            x_icon = QPixmap(16, 16)
            x_icon.fill(Qt.transparent)
            painter = QPainter(x_icon)
            pen = QPen(QColor('red'))
            painter.setPen(pen)
            ...
            fallback_box.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
            yes_button = fallback_box.button(QMessageBox.Yes)
            no_button = fallback_box.button(QMessageBox.No)
    ...
    def calculate_items_size(self, item_size):
        items_per_row = 16
        num_rows = 272
        width = items_per_row * item_size
        height = (num_rows * item_size) // items_per_row
        self.items_width = width
        self.items_height = height
        
    def calculate_blocks_size(self, block_size):
        blocks_per_row = 16
        num_rows = 544
        width = blocks_per_row * block_size
        height = (num_rows * block_size) // blocks_per_row
...
if __name__ == "__main__":
    ...
    main_window.fallback_on_launch()
    main_window.show()
    sys.exit(app.exec_())
